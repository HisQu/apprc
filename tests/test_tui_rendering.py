...
def test_build_field_table_rows_hides_keys_and_styles_declared_types() -> None:
    ...
    assert rows_by_key["DEMO_API_TOKEN"].cells[4].style == "dim italic"
    assert rows_by_key["DEMO_STRATEGY"].cells[3].plain == "shell"
    assert rows_by_key["DEMO_STRATEGY"].cells[5].style == "bold cyan"
    assert rows_by_key["DEMO_RETRY_COUNT"].cells[4].style == "yellow"
    assert rows_by_key["DEMO_CACHE_DIR"].cells[5].style == "green"
