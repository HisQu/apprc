...
def bind_owner_from_env(
    ...
    *,
    ...
    :param override_python_values: Whether env may replace Python-owned values.
    :param source: Explicit source snapshot, or the process environment.
    ...
    binding_env, skipped_python_fields = env_values_for_binding(
        ...
        values=source.values if source is not None else None,
    )
    ...
    for spec in owner.fields:
        ...
        bound_fields.append(
            EnvBoundField(
                ...
                value=loaded_value,
                origin=(
                    source.origins[env_key]
                    if source is not None
                    else ConfigOriginState(
                        "shell_export_variable", env_key=env_key
                    )
                ),
            )
        )
